...
if [[ -d "$hypr_config_dir" ]]; then
    ...
    mapfile -t dock_files < <(grep -l "exec.*nwg-dock-hyprland" "$hypr_config_dir"/*.conf 2>/dev/null)
    
    # Find all matching command lines
    mapfile -t dock_commands < <(grep -h "exec.*nwg-dock-hyprland" "$hypr_config_dir"/*.conf 2>/dev/null)
...
if [[ -n "$user_dock_command" ]]; then
    eval "$user_dock_command" & disown
else
    eval "$default_dock_command" & disown
fi
